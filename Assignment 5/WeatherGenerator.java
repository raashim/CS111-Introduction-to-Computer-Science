...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
       ...
       int column = 0;
        ...
        for(int i = 0; i < drywet.length; i++) {
                if( (longitude == drywet[i][column]) && (latitude == drywet[i][column+1]) ) {
                    for(int j = 0; j < 12; j++) {
    ...
    for(int i = 0; i < wetwet.length; i++) {
        if( (longitude == wetwet[i][column]) && (latitude == wetwet[i][column+1]) ) {
            for(int j = 0; j < 12; j++) {
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
        ...
        int [] arr = forecastGenerator(drywetProbability[month], wetwetProbability[month], numberOfDaysInMonth[month]);
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
       ...
       int count = 0; 
        ...
        for(int i = 0; i < forecast.length; i++) {
            ...
            if(tempLargestCount < count) {
                tempLargestCount = count; 
            }
        }

        return tempLargestCount; 
    }
